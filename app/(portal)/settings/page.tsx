...
// Staff: supabase.from('users').select('*').eq('org_id', orgId)
const mockStaff = [
...
/**
 * Settings page — facility profile, staff management, document categories.
...
export default function SettingsPage() {
  ...
  const handleSaveOrg = async (e: React.FormEvent) => {
    ...
    // TODO: supabase.from('organisations').update(orgForm).eq('id', orgId)
  ...
  const handleAddCategory = () => {
    ...
    // TODO: supabase.from('document_categories').insert({ org_id: orgId, name: val })
    setNewCategory('')
  }

  const handleRemoveCategory = (cat: string) => {
    setCategories(prev => prev.filter(c => c !== cat))
    // TODO: supabase.from('document_categories').delete().eq('name', cat).eq('org_id', orgId)
  }
  ...
  return (
    <div className="max-w-3xl">
      <PageHeader
        title="Settings"
        subtitle="Facility profile, staff management, and document categories"
      />
            ...
            {[
              ...
              <div key={field.id}>
                ...
                <input
                  ...
                  onChange={e =>
                    setOrgForm(prev => ({ ...prev, [field.key]: e.target.value }))
                  }
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {categories.map(cat => (
            <span
              ...
              style={{ borderColor: '#ddd6c8', color: '#1a1a1a', backgroundColor: '#F5F0E8' }}
            >
              {cat}
              <button
                type="button"
