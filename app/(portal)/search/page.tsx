...
// TODO: Replace with real Supabase full-text search queries once credentials are configured:
// Documents: supabase.from('documents').select('*').textSearch('search_vector', query, { type: 'websearch' }).eq('org_id', orgId)
// OR: supabase.rpc('search_documents', { query_text: q, org_id: orgId })
// Patients: supabase.from('patients').select('*').ilike('full_name', `%${q}%`).eq('org_id', orgId)
// Sections: supabase.from('document_sections').select('*').ilike('title', `%${q}%`)
...
/**
 * Search results page — queries documents, residents, and sections for the given term.
 * Results are grouped by type with clear headings.
 */
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  return (
    <div>
      ...
      {!query && (
        <div
          className="bg-white rounded-xl border py-16 flex flex-col items-center text-center"
        ...
        >
          ...
          <p className="text-xs mt-1" style={{ color: '#5a5a5a' }}>
            Use the search bar in the navigation to search across documents, residents, and sections.
          </p>
        </div>
      )}
              ...
              <div className="space-y-2">
                {sections.map(s => (
                  ...
                  >
                    ...
                    <a
                      href={`/pillar/${s.pillar.replace('_', '-')}`}
