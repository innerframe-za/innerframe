...
// TODO: replace all mock data below with Supabase queries once credentials are configured
// Query: supabase.from('patients').select('count').eq('org_id', orgId)
const mockStats = {
  totalResidents: 24,
  totalDocuments: 87,
  pendingReviews: 3,
...
// TODO: replace with Supabase query: supabase.from('documents').select('count').eq('org_id', orgId).eq('pillar', p)
const mockPillarCounts = {
  admin: 22,
  finance: 18,
  kitchen: 15,
  medical: 20,
...
// TODO: replace with Supabase query: supabase.from('documents').select('*').eq('org_id', orgId).order('created_at', { ascending: false }).limit(5)
const mockDocuments = [
...
// TODO: replace with Supabase query: supabase.from('patients').select('*').eq('org_id', orgId).order('created_at', { ascending: false }).limit(5)
const mockResidents = [
...
/**
 * Dashboard / Admin Office page — the first screen after login.
 * Shows stats, pillar quick-nav, recent documents, and recent residents.
 */
export default function DashboardPage() {
  return (
    <div>
      ...
      {/* Pillar Overview */}
      <div className="mb-6">
        <h2 className="text-sm font-medium mb-3" style={{ color: '#5a5a5a' }}>
          Pillar Overview
        </h2>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        >
          <div className="flex items-center justify-between mb-4">
            <div>
              <h3 className="text-sm font-medium" style={{ color: '#1E3A2F' }}>
                Recent Documents
              </h3>
              <div
