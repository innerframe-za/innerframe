...
export default function ResidentsPage() {
  ...
  const filtered = mockResidents.filter(r => {
    const matchSearch = r.name.toLowerCase().includes(search.toLowerCase()) || r.roomNumber.includes(search)
    ...
    <div>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-5">
        <div className="relative flex-1 max-w-sm">
          ...
          <input type="search" value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by name or room..."
        ...
        <select value={filterStatus} onChange={e => setFilterStatus(e.target.value)} className="px-3 py-2.5 rounded border text-sm outline-none" style={{ borderColor: '#ddd6c8', backgroundColor: '#ffffff', color: '#1a1a1a' }}>
      ...
      <div className="bg-white rounded-xl border overflow-hidden" style={{ borderColor: '#ddd6c8', borderWidth: '0.5px' }}>
        <div className="hidden md:grid grid-cols-[1fr_120px_120px_130px_120px_80px] gap-4 px-5 py-3 border-b" style={{ borderColor: '#ddd6c8', backgroundColor: '#F5F0E8' }}>
        ...
        {filtered.length === 0 ? (
          <div className="py-12 text-center"><p className="text-sm" style={{ color: '#5a5a5a' }}>No residents found matching your search.</p></div>
        ) : (
          filtered.map((resident, index) => {
            const sc = statusConfig[resident.status]
            return (
              <div key={resident.id} className="grid grid-cols-1 md:grid-cols-[1fr_120px_120px_130px_120px_80px] gap-2 md:gap-4 px-5 py-3.5 border-b last:border-b-0 items-center" style={{ borderColor: '#ddd6c8', backgroundColor: index % 2 === 0 ? '#ffffff' : '#fafaf9' }}>
                <div className="flex items-center gap-3">
                  <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-medium text-white flex-shrink-0" style={{ backgroundColor: '#1E3A2F' }} aria-hidden="true">
                    {resident.name.split(' ').map(n => n[0]).slice(0, 2).join('')}
                  </div>
                  <span className="text-sm font-medium" style={{ color: '#1a1a1a' }}>{resident.name}</span>
                </div>
                <span className="text-sm font-mono" style={{ color: '#5a5a5a' }}>{resident.idNumber}</span>
                <span className="text-sm" style={{ color: '#1a1a1a' }}>Room {resident.roomNumber}</span>
                <span className="text-sm" style={{ color: '#5a5a5a' }}>{resident.admissionDate}</span>
                <div>
                  <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full text-xs font-medium" style={{ backgroundColor: sc.bg, color: sc.color }}>
                    <span className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: sc.dot }} aria-hidden="true" />{sc.label}
                  </span>
                </div>
                <Link to={`/residents/${resident.id}`} className="text-xs font-medium transition-colors" style={{ color: '#1E3A2F' }}
