...
const mockResidents: Record<string, {
  id: string; name: string; dob: string; idNumber: string
  roomNumber: string; admissionDate: string; status: 'active' | 'discharged' | 'deceased'
}> = {
...
export default function ResidentDetailPage() {
  ...
  const resident = id ? mockResidents[id] : null

  if (!resident) return <Navigate to="/residents" replace />
  ...
  return (
    <div>
      <PageHeader
        ...
        action={
          <button type="button" className="px-4 py-2 rounded border text-sm font-medium" style={{ borderColor: '#1E3A2F', color: '#1E3A2F' }}>
            Edit Resident
          </button>
        }
      />

      <div className="bg-white rounded-xl border p-6 mb-6" style={{ borderColor: '#ddd6c8', borderWidth: '0.5px' }}>
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-sm font-medium" style={{ color: '#1E3A2F' }}>Personal Details</h2>
            ...
            return (
              <div key={field.label}>
                <div className="flex items-center gap-1.5 mb-1">
                  ...
                  <span className="text-xs" style={{ color: '#5a5a5a' }}>{field.label}</span>
                </div>
                <p className="text-sm font-medium" style={{ color: '#1a1a1a' }}>{field.value}</p>
              </div>
            )
          })}
        </div>
      ...
      <div className="bg-white rounded-xl border p-6" style={{ borderColor: '#ddd6c8', borderWidth: '0.5px' }}>
        <div className="flex items-center justify-between mb-5">
          <div>
            <h2 className="text-sm font-medium" style={{ color: '#1E3A2F' }}>Documents</h2>
            <div style={{ width: '36px', height: '2px', backgroundColor: '#D4AF37', marginTop: '4px' }} aria-hidden="true" />
          </div>
          <button type="button" className="inline-flex items-center gap-2 px-3 py-2 rounded text-xs font-medium" style={{ backgroundColor: '#1E3A2F', color: '#ffffff' }}>
              ...
              <TabsContent key={p.value} value={p.value}>
                {docs.length === 0 ? (
                  ...
                  <div className="space-y-2">
                    {docs.map(doc => <DocumentRow key={doc.id} fileName={doc.fileName} fileUrl={doc.fileUrl} category={doc.category} pillar={doc.pillar} date={doc.date} isGlobal={doc.isGlobal} canDelete={true} />)}
                  </div>
                )}
              </TabsContent>
            )
